...
Service와 데이터 정리에 필요한 Repository를 주입한다. 정리 로직은 `@AfterEach`에 둔다.
...
### 1-3. 클래스 레벨 `@Transactional`을 붙이지 않는다 ⚠️

- Repository 테스트와 **다른 점**이다. Service 테스트는 `@Transactional`을 쓰지 않는다.
- 이유:
  - Service 자체가 트랜잭션 경계를 가지므로, 테스트가 트랜잭션을 한 번 더 감싸면
    **실제 운영 환경과 다른 트랜잭션 구조**가 된다. 영속성 컨텍스트 1차 캐시,
    `@Transactional(readOnly = true)` 동작, 이벤트 발행 시점 등이 달라질 수 있다.
  - 테스트가 무심코 `@Transactional` 안에서 통과해버려서 **실제로는 영속화되지 않는
    버그**를 못 잡는다.

### 1-4. `@AfterEach`로 데이터를 정리한다

→ 코드 예시: reference.md §1, §2 (여러 Repository를 정리하는 케이스)
...
- [ ] `IntegrationTestSupport`를 상속하는가?
- [ ] 클래스 레벨 `@Transactional`이 **없는가**? (있으면 잘못된 신호)
- [ ] `@AfterEach`에서 `deleteAllInBatch()`로 정리하는가?
- [ ] 삭제 순서가 외래키 참조 관계에 맞는가?
