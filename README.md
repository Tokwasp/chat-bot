...
└───────────────────────┬──────────────────┬───────────────────────────┘
                        │                  │
              POST /api/chat         POST|GET|PUT|DELETE
             (SSE 스트리밍)           /api/sessions/**
                        │                  │
┌───────────────────────▼──────────────────▼───────────────────────────┐
│                        Spring Boot Backend                            │
│                                                                      │
...
│  │  (SseEmitter 반환)        │   │   (세션 CRUD REST API)          │   │
│  └────────────┬────────────┘   └────────────────────────────────┘   │
│               │                                                      │
│  ┌────────────▼────────────────────────────────────────────────┐    │
│  │                        ChatService                           │    │
...
│  │   3. 시스템 프롬프트(페르소나) 적용                              │    │
│  │   4. Bedrock 스트리밍 호출                                     │    │
│  │   5. SSE 이벤트 전송 (text / tool_start / done / error)       │    │
│  │   6. 어시스턴트 응답 저장                                       │    │
│  └────────────┬─────────────────┬────────────────────────────--┘    │
│               │                 │                                    │
│  ┌────────────▼───┐   ┌─────────▼──────────┐                        │
│  │ BedrockService  │   │ MessageHistory      │                        │
│  │ (SDK 타입 변환)  │   │ Service             │                        │
│  └────────┬───────┘   │ (히스토리 저장/트림)  │                        │
│           │            └─────────┬──────────┘                        │
│           │                      │                                   │
│  ┌────────▼──────────────────────▼───────────────┐                  │
│  │   BedrockRuntimeAsyncClient (AWS SDK v2)       │                  │
│  └────────┬──────────────────────────────────────┘                  │
│           │                    ┌──────────────────────────────┐     │
│           │                    │   H2 / MySQL Database         │     │
│           │                    │   - sessions 테이블            │     │
...
```
Client                  ChatController           ChatService              AWS Bedrock
  │                          │                        │                        │
  │── POST /api/chat ────────▶│                        │                        │
  │                          │── stream(emitter) ─────▶│                        │
  │◀── SSE Connection ───────│                        │── converseStream() ────▶│
  │                          │                        │                        │
  │                          │                        │◀── text_delta ─────────│
  │◀── event: text ──────────────────────────────────│                        │
  │       {"text": "안"}      │                        │◀── text_delta ─────────│
